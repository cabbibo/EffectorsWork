...
float3 cubicFromValue( in float val , out float3 upPos , out float3 doPos , out float3 vel ){
  ...
  return pos;
			...
			float getRadius( float row , float3 vel){
					return	.01 + length(vel)  * clamp( .5 - abs( row - .5) , 0 , 1)  * (sin( row * 30. + _Time.y) + 3.);
			}
...
//
            ...
            float4 frag (varyings v) : COLOR {
              ...
              fCol = cubeCol * 2* (fRefl*.5+.5);

              //fCol=_FireInfo.z * 1/(1+ 100*abs(_FireInfo.x - v.uv.x));
