...
Blend SrcAlpha OneMinusSrcAlpha
    ...
    #include "Chunks/snoise.cginc"
      ...
      VertexOut vert(VertexIn v) {
        ...
        float n =  snoise( float3(v.texcoord.xy * 100,_Time.y) ) * .03;
  		float3 fPos = v.position + v.normal * n ;
        ...
        o.rd = normalize( o.camPos - fPos.xyz );
      ...
      fixed4 frag(VertexOut i) : COLOR {
        ...
        float m = dot( rd , i.normal );

        float noise = i.debug.x;
        ...
        col = lerp( bw * bw * 4,col,_Saturation);
		 ...
		 fixed4 color;
        color = fixed4( float3(1,1,1) , pow(1.-m,6) ) * noise * 10.;
        return color;
