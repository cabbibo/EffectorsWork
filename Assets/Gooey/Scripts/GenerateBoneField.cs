...
public class GenerateBoneField : MonoBehaviour {
  ...
  private int pointStructSize = 1+3+3+3+2+3;
    ...
    for( int i = 0; i < numberOfLimbs; i++ ){
      ...
      int tri0 = (int)(randomVal * (float)(baseTriBuffer.values.Length/3)) * 3;
      ...
      tri0 = baseTriBuffer.values[tri0];
      tri1 = baseTriBuffer.values[tri1];
      tri2 = baseTriBuffer.values[tri2];
    ...
    for( int i = 0; i < numberOfLimbs; i++ ){
      ...
      for( int j = 0; j < numberOfBonesPerLimb; j++ ){
        ...
        var tmpMat = Matrix4x4.Rotate(boneBuffer.bones[j].transform.rotation) * boneBuffer.bindPoses[j] ; 
  ...
  void OnRenderObject(){
    ...
    Graphics.DrawProcedural(MeshTopology.values, 3  * totalNumberPoints );*/
    ...
    material.SetInt("_VertsPerMesh" , vertBuffer.count);
    material.SetInt("_TriCount", triBuffer.count );
    material.SetInt( "_TotalVerts", totalNumberVerts );

    material.SetVector("_Color1", color1);
    material.SetVector("_Color2", color2);

    Graphics.DrawProcedural(MeshTopology.Triangles, triBuffer.count * numberOfLimbs );
